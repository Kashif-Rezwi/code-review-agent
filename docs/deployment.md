...
| `PAYMENTS_DEV_PACK` | *(Optional)* Operator-chosen random secret that enables the hidden ₹1 `dev1` pack for cheap live-mode smoke testing; unlock per-request via `/account?dev_pack=<same secret>`. Never commit or document the value |
