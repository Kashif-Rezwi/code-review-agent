...
export function ReviewPageClient({ initialReviewType, initialReviewId }: { initialReviewType?: string, initialReviewId?: string }) {
    ...
    useEffect(() => {
        if (sessionData && sessionData.type) {
            ...
            // We NO LONGER set mode here because the URL strictly dictates the mode via initialReviewType
            if (isCode) {
    ...
    const handleModeSwitch = (m: Mode) => {
        ...
        setMode(m)
        reset()
        // Update URL to match current mode without forcing a React remount
