...
> **Status:** pending · **Findings:** C-1, S-4, S-5, M-3, A-31, A-32 (6) · **Severity mix:** 🟠4 🟡2
...
1. [ ] **A-31 + A-32 (ungated doc fixes, do first).** Update `docs/authentication.md`: document the `?token=` fallback **with a security warning** (tokens in URLs land in proxy/access logs, browser history, referrers), and correct the cache description ("at 500 entries, expired entries are evicted; live entries are not bounded" — or the new behavior if task 4 lands first). **Acceptance:** both claims match code.
2. [ ] **C-1 — drop `repo` scope (needs Q2 = "no user-token private-PR plans").** Change scope to `'read:user user:email'` in `lib/auth.ts` and fix the misleading comment. **Acceptance:** fresh OAuth flow requests only the two scopes; login works; PR review of a public repo still works (uses server `GITHUB_TOKEN`). Note: existing users' tokens keep old scopes until re-auth — mention this in PROGRESS.md.
3. [ ] **S-4 — deprecate `?token=` (needs Q5).** Phase 1 (this chunk): add a `Logger.warn` deprecation log when the query-param path is used in `auth.guard.ts`, and keep the docs warning from task 1. Removal happens later only if logs show zero usage. **Acceptance:** using `?token=` logs a deprecation warning; header path unaffected.
4. [ ] **S-5 — enforce a real bound.** After evicting expired entries, if size is still ≥ 500, evict oldest-inserted entries (Map preserves insertion order) until under the cap. Update `docs/authentication.md` to match. **Acceptance:** unit test — inserting 501 live entries keeps size ≤ 500.
...
6. [ ] **M-9 (CORS sub-item).** Stop trusting localhost in prod: `origin: process.env.NODE_ENV === 'production' ? [frontendUrl] : [frontendUrl, 'http://localhost:3000']`. **Acceptance:** prod config rejects `Origin: http://localhost:3000`; dev unaffected.
...
- [ ] Q2/Q3/Q5 answers recorded in this file
- [ ] OAuth scope narrowed (or decision recorded)
- [ ] `?token=` deprecation log + docs warning live
- [ ] Token cache hard-bounded + tested; docs corrected
- [ ] Rate limiting implemented or explicitly deferred with rationale
- [ ] Prod CORS tightened
- [ ] `PROGRESS.md` updated (6 findings)
